...
import { householdList } from '@/lib/catalogue';
import { font, radius } from '@/lib/theme';
import { useTheme } from '@/lib/theme-context';
import { useWizard } from '@/lib/wizard-context';

// furniture, trailers, kitchen and diabetic supplies all pick the same way:
// one card, then its models. only the copy and the list differ
export default function PickerScreen() {
  ...
  const list = householdList(state.category);
  if (!list) return <Redirect href="/new-search" />;

  const q = query.trim().toLowerCase();
  const shown = q ? list.entries.filter((e) => e.name.toLowerCase().includes(q)) : list.entries;
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.content}>
        <ScrollView
          ...
          <View style={[styles.keywordCard, { backgroundColor: colors.accentFaint, borderColor: colors.accentChip }]}>
            <View style={styles.cardText}>
              <Text style={[styles.keywordTitle, { color: colors.textPrimary }]}>{list.notListed}</Text>
      ...
      <WizardBar
        ...
        disabled={!list.entries.some((e) => e.brand.id === state.brandId)}
      />
    </View>
  );
}
